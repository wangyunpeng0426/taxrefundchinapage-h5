<template>
  <div class="content-3">
    ...
    <div class="content-bottom">
      ...
      <div class="block-cash">
        <div class="guarantee-amount">
          预计担保退税金（人民币）：<span v-if="voucherTotal"
            ><span class="amount">{{ $toFixed2(voucherTotal) }}</span
            >元</span
          ><span v-else>--</span>
          <!-- {{
            voucherTotal ? `${$toFixed2(voucherTotal)}元` : "--"
          }} -->
        </div>
      </div>
      <!-- <div class="block-2">
    ...
    <div class="btn-group-bottom-container">
      <div class="logo-icbc">
        <orgLogo></orgLogo>
      </div>
      <div class="btn-group">
...
export default {
  ...
  methods: {
    ...
    toSign() {
      Dialog.confirm({
        title: "操作提示",
        message: `确认担保后，将对您信用卡进行冻结人民币${this.$toFixed2(this.voucherTotal)}元，最长冻结90天。确定执行操作嘛？`,
        confirmButtonText: this.$t("Receipt.dialog_confirm"),
        cancelButtonText: this.$t("App.dialog_cancel"),
        confirmButtonColor: "#3775F6",
        cancelButtonColor: "#3775F6",
      })
        .then(() => {
          this.$router.push({
            path: "/Sign",
