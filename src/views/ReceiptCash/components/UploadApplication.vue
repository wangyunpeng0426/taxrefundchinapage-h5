  ...
  <div>
    ...
    <div class="content-1">
      <div class="content-middle">
        ...
        <van-cell
          v-if="'1' === bankCardType || 'VISA' === issueOrgCode"
      ...
      <div
        ...
        :style="{
          'min-height':
            '1' === bankCardType || 'VISA' === issueOrgCode
              ? '5rem'
              : '6.08rem',
        }"
      >
    ...
    <van-dialog v-model="dialog_prompt" :show-confirm-button="false">
      ...
      >
        <div style="margin-left: 16px">
          <span style="color: #ff9903; font-weight: bolder; font-size: 16px"
            ><van-icon name="question-o"
          /></span>
          操作确认
        </div>
        <div style="margin-right: 16px" @click="dialog_prompt = false">
          <van-icon name="cross" />
        </div>
      </div>
      <div style="width: 100%; padding: 0.5rem 0.4rem 0.5rem 0.4rem">
        当前税单总退税额已经超过人民币{{cardLimit.toFixed(2)}}元，根据规定仅能退款到银行卡。您是否确认？
      </div>
      <div
        style="
          width: 100%;
      ...
      >
        <van-button
          ...
          @click="dialog_prompt = false"
          >{{$t("App.dialog_cancel")}}</van-button
        >

        <van-button color="#409EFF" size="small" @click="switchBankCard"
          >退款银行卡</van-button
        >
      </div>
    </van-dialog>
  ...
  methods: {
    ...
    async getCardDefault() {
      ...
      if (response.code == "success") {
        ...
        this.cardLimit = Number(response.data.cashTaxCardLimit) || 0;
        // this.cardLimit = 200;
        if ("1" === this.bankCardType || "VISA" === this.issueOrgCode) {
          if (this.total) {
            this.limit = Number(this.cardLimit - this.total).toFixed(2);
    ...
    switchBankCard() {
      this.dialog_prompt = false;
      this.setTaxRefundType("BackCard");
      this.$router.push({
        path: `/Receipt`,
        query: {
          touristId: this.touristId,
